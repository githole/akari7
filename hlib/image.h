    ...
    static bool load(const char *filename, Image *image) {
        ...
        for (;index < rest_size;) {
            ...
            if (now == 0x02 && now == 0x02) {
                ...
                const int now3 = buffer[index++];
                const int now4 = buffer[index++];
                tmp_data[(nowy * width + basex) * 4 + 0] = now;
                tmp_data[(nowy * width + basex) * 4 + 1] = now2;
                tmp_data[(nowy * width + basex) * 4 + 2] = now3;
                tmp_data[(nowy * width + basex) * 4 + 3] = now4;
                ++basex;
                if (basex >= width) {
                    basex = 0;
                    ++nowy;
