...
public:
    ...
    static Vec3<T> uniformSphereSurface(T u, T v)
    {
        const T tz = u * 2 - 1;;
        const T phi = v * 2.0f * kPI;
	...
	static Vec3<T> uniformHemisphereSurface(Random &random, const Vec3<T> &normal, const Vec3<T> &tangent, const Vec3<T> &binormal, float angle = kPI / 2) {
		const T tz = random.next(cos(angle), 1.0f);
		const T phi = random.next(0.0f, 2.0f * kPI);
		const T k = sqrt(1.0f - tz * tz);
		const T tx = k * cos(phi);
		const T ty = k * sin(phi);
