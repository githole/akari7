...
//		tris = atris;
    ...
    static __m128 _mm_not_ps(__m128 p_para) {
        __m128 t_t1 = {};
	...
	bool intersect(const Ray &ray, Hitpoint* hitpoint, int selfid, bool anyhit = false, float near_t = 0, float far_t = 0) const {
        ...
        alignas(16) float near_t_data[4];
        alignas(16) float far_t_data[4];
        std::fill(near_t_data, near_t_data + 4, near_t);
        std::fill(far_t_data, far_t_data + 4, far_t);
        __m128 near_t_sse = _mm_load_ps(near_t_data);
        __m128 far_t_sse = _mm_load_ps(far_t_data);

        alignas(64) float dist[4];
        std::fill(dist, dist + 4, hitpoint->distance);
        __m128 current_distance = _mm_load_ps(dist);
		...
		while (todoNode >= 0) {
			...
			if(item.node.flag == 0){
...
#if 0
                auto hit_flag = _mm_and_ps(_mm_not_ps(no_hit), _mm_and_ps(_mm_and_ps(_mm_cmplt_ps(near_t_sse, t), _mm_cmplt_ps(t, far_t_sse)), _mm_cmplt_ps(t, current_distance)));
                int hit_flag_int = _mm_movemask_ps(hit_flag);
                if (hit_flag_int == 0)
                    continue;
                ...
                if (k)
                {
                    alignas(64) float dist[4];
                    std::fill(dist, dist + 4, hitpoint->distance);
                    current_distance = _mm_load_ps(dist);
                }
#endif
#if 1
				...
				for (int i = 0; i < 4; ++i) {
					...
					if ((nohitmask & (1 << i)) == 0 && hitpoint->distance > current_t) {
                        if (near_t <= current_t && current_t <= far_t && selfid != s->idx[i])
                        {
                            hit = true;
                            triangle_index = s->idx[i];
