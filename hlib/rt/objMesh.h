...
public:
	static bool load_material(const char *filename, MaterialMap *matmap, MaterialVector *matvec) {
		FileManager manager;
		...
		for (;;) {
			...
			if (ret[0] == "newmtl") {
                ...
                auto m = Material(diffuse, specular, specular_coefficient, metalic);
				matmap->insert(std::pair<std::string, int32_t>(now_material_name, matvec->size()));
                matvec->push_back(m);
				now_material_name = "";
	...
	static bool load(const char *filename, TriangleMesh *mesh) {
		...
		for (;;) {
			...
			if (ret[0] == "mtllib") {
				// マテリアルの読み込み
				if (!load_material(ret[1].c_str(), &mesh_body.matmap, &mesh_body.matvec))
					return false;
